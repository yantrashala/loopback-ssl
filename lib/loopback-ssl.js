...
var getServerOptions = function (cfg) {
  ...
  // if httpMode==false and path, key and cert is blank, generate self signed certificate
  if (cfg.path === "" && cfg.key === "" && cfg.cert === "") {
    pem.createCertificate({days:364, selfSigned:true}, function(err, keys){
      options = {
        key: keys.serviceKey,
